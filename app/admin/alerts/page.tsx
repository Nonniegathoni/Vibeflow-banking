  ...
  const filteredAlerts = alerts.filter((alert) => {
    const matchesSearch = searchQuery
      ? alert.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        alert.transaction_reference?.toLowerCase().includes(searchQuery.toLowerCase())
      : true

    const matchesStatus = statusFilter !== "all" ? alert.status === statusFilter : true
                ...
                <TableBody>
                  {currentAlerts.map((alert) => (
                    <TableRow key={alert.id}>
                      ...
                      <TableCell>{formatCurrency(alert.transaction_amount)}</TableCell>
                      <TableCell>
                        <span
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => router.push(`/admin/transactions/${alert.transaction_id}`)}
                          >
                            View
                          </Button>
                          {alert.status !== "resolved" && (
